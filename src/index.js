...
function merge(opt) {                   //1 проходка на слияние
    var new_angles = [];                                 // здесь мы формируем новый эррей со слитыми треугольниками
    var triang_1 = [];
    var triang_2 = [];
    var angles = [];
    var sum_angles = [];
    for (var i = 1; i < opt.length; i++) {              //берем отверстие 
        for (var j = i + 1; j < opt.length; j++) {
            triang_1 = [opt[i][0][0], opt[i][0][1], opt[i][1][0], opt[i][1][1]];            //эррей с 4 реальными координатами треугольника [x1, y1, x2, y2];
            triang_2 = [opt[j][0][0], opt[j][0][1], opt[j][1][0], opt[j][1][1]];
            if (checkOverlap(triang_1, triang_2)) {
                console.log("Отверстия " + i + " и " + j + " накладываются");
                angles = [opt[i][0][2], opt[i][1][2], opt[j][0][2], opt[j][1][2]];          // углы наложившихся треугольников (уг1_1, уг1_2, уг2_1, уг2_2);
...
function mergeOpenings(opening_tangents) {              //пробуем слить пересекающиеся все треугольники касательных  в один
    ...
    prelim_result_1 = merge(opening_tangents);                  //делаем первую проходку - проходимся по всем углам, пробуем найти наложение
    // если наложения нашли, значчит мы получили новый треугольник (и удалили два старых) - следовательно нужно еще раз пропустить этот блок кода 
    // (вдруг новый треугольник перексекается еще с кем то) - следовательно гоняем этот блок кода, до тех пор, пока не найдем все пересечения
    while (prelim_result_1.length > 0) {
        prelim_result_2 = [...prelim_result_1];                     // снимаем копию с текущего результата
        prelim_result_1 = merge(prelim_result_1);
    }
    ...
    if (prelim_result_1.length === 0) {                                 // если слияние не нашли (или слияний больше нет)
        ...
        if (prelim_result_2.length > 0) {
            angles = [];
            for (var i = 0; i < prelim_result_2.length; i++) {              //переводим результат в одномерный эррей
                if (prelim_result_2[i].length >= 2) {                      
                    angle1 = prelim_result_2[i][0];                    
                    angle2 = prelim_result_2[i][1];                     
                    angles.push(angle1);
                    angles.push(angle2);
                }
            }                      
            return angles;                           // если находили пересечение ранее, то выдаем последний полученный эррей
        }
    }  
}
...
var test = [
    [],
    [[850, 450, 62.103], [-650, 450, 304.695]],
    [[-450, 1400, 342.181], [-450, -100, 257.471]],
    [[625, -425, 124.216], [375, -775, 154.179]],
    [[575, -225, 111.371], [425, -575, 143.531]]
];

var test_res = mergeOpenings(test); 
/*
var test_res_2 = mergeAngles(test);
*/
console.log(test_res);
/*
console.log(test_res_2);
*/

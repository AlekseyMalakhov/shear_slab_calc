...
function findAngleReal(coords) {                                      //находим углы относительно 0,0. findAngle и findAngleReal эти две функции одинаковые по сути и их можно (НУЖНО!) в будущем объединить в одну. Щас просто нет времени 
    ...
    return Number(result.toFixed(3));                    //округляем до 3 знака после запятой
}
...
function mergeAngles(opening_tangents) {                //пробуем слить пересекающиеся треугольники касательных  в один
    var prelim_result_1 = [];                                   // предварительный результат 1 - эррей с углами
    var prelim_result_2 = [];                                   // предварительный результат 2 - эррей с углами
    var angle1;
    var angle2;
    ...
    prelim_result_1 = findTrianglesIntersect(angles);                  //делаем первую проходку - проходимся по всем углам, пробуем найти наложение
    ...
    while (prelim_result_1.length > 0) {
        prelim_result_2 = [...prelim_result_1];                     // снимаем копию с текущего результата
        prelim_result_1 = findTrianglesIntersect(prelim_result_1);
    }

    if (prelim_result_1.length === 0) {                                 // если слияние не нашли (или слияний больше нет)
        if (prelim_result_2.length === 0) {                         // проверяем не находили ли мы его ранее
            return angles;                                     // если не находили и ранее, значит выдаем исходный эррей - слияние не обнаружено вообще;
        }
        if (prelim_result_2.length > 0) {                      
            return prelim_result_2;                           // если находили пересечение ранее, то выдаем последний полученный эррей
        }
    }
    
}

function findTrianglesIntersect(angles) {                           
    // функция берет список углов касательных и проверяет их на наличие наложений. Она делает одну проходку и при нахождении 
    // наложений объединяет накладывающиеся треугольники в один. Затем удаляет входящие в наложение треугольники и вместо них вставляет реузльтирующий. 
    // Результирующий эррей она не проверяет. Чтобы проверить реузльтирующий эррей на пересечения (вдруг остались еще неслитые треугольники) нужно запустить findTrianglesIntersect еще раз и передать ей результирующий эррей
    var filtered = [];                          // наш результат
    var tang0, tang1, tang2, tang3;                      //вспомогательные переменные обозначающие проверяемые касательные
    var triang_fin = [];                                  // итоговый треугольник получаемый после слияния двух пересекающихся треугольников
    var new_angles = [];                                 // здесь мы формируем новый эррей со слитыми треугольниками
    
    for (var j = 0; j <= angles.length - 2; j = j + 2) {              //берем блок отверстия состоящий из двух углов касательных
        for (var m = j + 2; m <= angles.length - 1; m = m + 2) {               // и берем по очереди другие блоки (если они есть)
            // если угол касательной 0 принадлежит промежутку углов 2;3 или угол касательной 1 принадлежит промежутку углов 2;3, это значит, что треугольнкики отверстий 0 и 1 пересекаются. 
            // Углы касательных отверстий 0 и 1 сливаются в один итоговый эррей, [0, 1, 2 ,3] из которого формируется итоговый треугольник 
            // полученный слиянием треугольников 1 и 2. Стороны треугольника определяются как MIN/MAX из углов итогового эррея [0, 1, 2 ,3].

            // min_angle = Math.min(...angles[i]);
            ...
            console.log("j = " + j + ", m = " + m);
            tang0 = Math.min(angles[j], angles[(j+1)]);                 //отверстие 1      
            tang1 = Math.max(angles[j], angles[(j+1)]);
            tang2 = Math.min(angles[m], angles[(m + 1)]);                //отверстие 2
            tang3 = Math.max(angles[m], angles[(m + 1)]);

            console.log(tang0 + ", " + tang1 + ", " + tang2 + ", " + tang3);
            

            if (!((tang0 < 90) && (tang1 > 270)) &&         //если первое отверстие не лежит на половину в 1 на половину во 2 квадранте
                !((tang2 < 90) && (tang3 > 270))            // и если второе отверстие не лежит на половину в 1 на половину во 2 квадранте
            ) {         //если стандартный случай, т.е. сравниваемые отверстия полностью лежат в 1, 2, 3 или 4 квадранте
                if (                    //проверка пересечения треугольников касательных к отверстиям - стандартный случай
                    (   //блок 1
                        (    //линия 0 находится между линийей 2 и 3
                            (tang0 >= tang2) && (tang0 <= tang3)   
                        ) || (   //или линия 1 находится между линией 2 и 3
                            (tang1 >= tang2) && (tang1 <= tang3) 
                        )
                    ) || (  //или блок 2
                        (   // линия 2 находится между линией  0 и 1 
                            (tang2 >= tang0) && (tang2 <= tang1)
                        ) || (  // или линия 3 находится между линией  0 и 1
                            (tang3 >= tang0) && (tang3 <= tang1)
                        )

                    )
                ) {
                    console.log("Отверстия " + j/2 + " и " + m/2 + " пересекаются");            // значит что треугольники пересекаются
                    //создаем итоговый треугольник
                    ...
                    //удаляем треугольники j и m из старого эррея angles - они нам теперь не нужны. Вместо них будет один большой слитый треугольник
                    new_angles = [...angles];               //снимаем копию со старого эррея
                    console.log("Старые углы треугольников были такими " + new_angles);
                    console.log("Мы удалили углы треугольников " + j/2 + " и " + m/2);
                    console.log("Мы слили углы треугольников " + j/2 + " и " + m/2 + " в один большой треугольник");
                    console.log("Его углы " + triang_fin);
                    new_angles[j] = "";
                    new_angles[(j+1)] = "";
                    new_angles[m] = "";
                    new_angles[(m+1)] = "";

                    new_angles = [...new_angles, ...triang_fin];                //присоединяем к оставшемуся эррею новый объединенный треугольник

                    filtered = new_angles.filter(function(value, index, arr){               //удаляем пустые позиции
                        return value !== "";
                    });
                    console.log("Мы добавили новый слитый треугольник к оставшимся старым и получили и получили вот такие углы " + new_angles);
                    console.log("Возвращаем слитый эррей " + filtered);
                    return filtered;                   
                }
            } else {        // если отверстие лежит на половину в 1 и на половину во 2 квадранте
                /*
                if (!(                    //проверка пересечения треугольников касательных к отверстиям - нестандартный случай. Идем от обратного- определим когда отверстия НЕ пересекаются в нестандартном случае. Таких случаев 4
                    ((tang0 <= 90) && (tang1 >= 270) && (tang2 <= 90) && (tang3 <= 90) && (tang2 > tang0)) ||                // не пересекаются если: отверстие 1 лежит на половину в 1 на половину во 2 квадранте, а отверстие 2 полностью в 1 и угол касательной 2 больше чем угол касательной 0
                    ((tang0 <= 90) && (tang1 >= 270) && (tang2 >= 270) && (tang3 >= 270) && (tang3 < tang1)) ||              // не пересекаются если: отверстие 1 лежит на половину в 1 на половину во 2 квадранте, а отверстие 2 полностью во 2 и угол касательной 3 меньше чем угол касательной 1
                    ((tang2 <= 90) && (tang3 >= 270) && (tang0 <= 90) && (tang1 <= 90) && (tang0 > tang2)) ||              // не пересекаются если: отверстие 2 лежит на половину в 1 на половину во 2 квадранте, а отверстие 1 полностью во 1 и угол касательной 0 больше чем угол касательной 2
                    ((tang2 <= 90) && (tang3 >= 270) && (tang0 >= 270) && (tang1 >= 270) && (tang1 < tang3))                // не пересекаются если: отверстие 2 лежит на половину в 1 на половину во 2 квадранте, а отверстие 1 полностью во 2 и угол касательной 1 меньше чем угол касательной 3
                )) {
                    console.log("Отверстия " + j/2 + " и " + m/2 + " пересекаются. Нестандартный случай");            // значит что треугольники пересекаются
                */
               if (!(                    //проверка пересечения треугольников касательных к отверстиям - нестандартный случай. Идем от обратного- определим когда отверстия НЕ пересекаются в нестандартном случае.
                    ((tang0 <= 90) && (tang1 >= 270) && (tang2 > tang0) && (tang3 > tang0) && (tang2 < tang1) && (tang3 < tang1)) ||
                    ((tang2 <= 90) && (tang3 >= 270) && (tang0 > tang2) && (tang1 > tang2) && (tang0 < tang3) && (tang1 < tang3))
                )) {
                    console.log("Отверстия " + j/2 + " и " + m/2 + " пересекаются. Нестандартный случай");            // значит что треугольники пересекаются
                    ...
                    console.log("Проходка " + j + "/" + m + " - " + non_st.fst_quad);
                    console.log("Проходка " + j + "/" + m + " - " + non_st.snd_quad);
    ...
    console.log("Ничего не склеилось " + filtered);
    return filtered;
}
...
function addCornersU(coords, uRealCoords, uCornersAngles) {         //проверяем находятся ли реальные координаты углов u внутри треугольника касательных. 
    ...
    // формируем задание на проверку. Разбиваем coords на отдельные финальные отверстия у каждого из которых есть начальная точка вырубки и конечная.
    var inters = [];                                
    var k = 0;
    for (var m = 0; m < (coords.length - 1); m = m + 2) {
        inters[k] = [coords[m], coords[m+1]];
        k++;
    }
    // получаем эррей inters который содержит в себе эррей отверстий, каждое из которых содержит эррей из двух точек (начало - конец), каждая из которых содержит эррей из двух чисел - Х и У
    console.log(inters);

    //теперь берем каждое отверстие отдельно для каждого считаем количество выбитых углов
    ...
    for (var n = 0; n < inters.length; n++) {           
        console.log("Берем отверстие " + n);
        console.log("У отверстия " + n + " есть две точки с координатами " + inters[n][0] + " и " + inters[n][1]);
        ...
        //если координата двух точек имеют хоть одно совпадение - значит они лежат на одной прямой u. Если эта прямая имеет координату u контура, значил между точками можно просто вычислить расстояние вырубки
        if (
        ...
        } else { // если же обе координаты не совпадают, значит здесь мы перелетаем через угол u (или даже 2 угла u) и нам нужно это учесть при расчете вырубки.                                                                               
            for (var j = 0; j < u_corners.length; j++) {                            // Итак мы перелетели через угол. Проходимся по всем углам u. Проверяем, мы вышибаем 1 угол или 2 угла. 
...
class App extends React.Component {                 // это наш главные самый верхний компонент в котором соединяются все нижние компоненты и который выполняет все вычисления
    ...
    calculateU() {                                   //считаем длину расчетного контура и другие геометрические характеристики
        ...
        if (st.openingIsNear) {                             //если есть отверстие считаем некоторые характеристики вырубки
            ...
            console.log(cut_off_ibx);
            console.log(cut_off_iby);
            console.log(cut_off_sx);
            console.log(cut_off_sy);
            console.log(cut_xc);
            console.log(cut_yc);
        }
    ...
    calculate() {                                                               //здесь мы собрали все исходные данные и делаем расчет как в СП
        ...
        function calculateU(st) {                                   //считаем длину расчетного контура и другие геометрические характеристики
            var result = {                                          // результат будет содержать длину u и моменты сопротивления
                u: 0,
                wbx: 0,
                wby: 0,
           ...
           if (st.slab_edge_type === "l") {       //лево   проверка рядом расположенного левого края плиты. Если расстояние между левым краем расчетного контура и краем плиты становится меньше чем левый контур/2,
            ...
            if (st.slab_edge_type === "") {
                ...
                result.ibx = Math.floor(result.ibx);                   //округляем
                result.iby = Math.floor(result.iby);                   //округляем
                console.log("ibx = " + result.ibx + ", iby = " + result.iby);
                result.wbx = result.ibx/(lx/2);
                result.wby = result.iby/(ly/2);
                result.wbx = Math.floor(result.wbx);                           //округляем
                result.wby = Math.floor(result.wby);                   //округляем
                console.log("wbx = " + result.wbx + ", wby = " + result.wby);
            }
